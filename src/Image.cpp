 ...
 * 载入bmp文件
 */
void EGImage::LoadBitmap(char *filename)
{
 ...
 * 载入带alpha值的bmp文件
 */
void EGImage::LoadBitmapWithAlpha(char *filename,DWCOLOR keycolor)
{
	LoadBitmap(filename);
	if(m_pImageData == NULL)
		return;
 ...
 * 载入tga文件
 */
void EGImage::LoadTga(char *filename)
{
	FILE *fp;
	fp = fopen(filename , "rb");
	if(fp == NULL)
...
unsigned char* EGImage::LoadCompessedTga(char *filename)
{
	FILE *fp;
	BYTE bChar;
	int iCount;
...
unsigned char* EGImage::LoadUncompessedTga(char *filename)
{
	FILE *fp;
	tgaHeader tga;
	int iPos;
 ...
 * 载入带alpha值的tga文件
 */
void EGImage::LoadTgaWithAlpha(char *filename, DWCOLOR keycolor)
{
	LoadTga(filename);
	if(m_pImageData == NULL)
		return;
